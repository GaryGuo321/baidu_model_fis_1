...
$(function() {
	...
	/*
	使用了单例模式
	利用单例模式可以创造一个封闭的空间，使私有的属性和方法独立起来，可以避免私有的属性和方法被污染
	利用闭包，暴露出公有的可以被外部访问的属性和方法。
	要注意闭包的内存泄漏的问题
	 */
	var DropDown = (function() {
		...
		function init() {
			...
			/*这里定义单例代码*/
			return {
				publicMethod: function(element, relElement) {
	...
	//天气下拉显示效果
	DropDown.getInstance().publicMethod($(".weather"), $(".weather-frame"));
	//右侧用户设置，信息下拉显示效果
	DropDown.getInstance().publicMethod($(".h-r-a .login"), $(".user"));
	DropDown.getInstance().publicMethod($(".h-r-a .login-setting"), $(".user-setting"));
	DropDown.getInstance().publicMethod($(".h-r-a .more"), $(".more-product"));
	...
	var FrameSwitch = (function() {
		...
		function init() {
			// 单例代码
			return {
				contentNavClick: function(element, relElement, className) {
				...
				navSettingHover: function(element, relElement, changeBefore, changeAfter) {
	...
	//点击导航栏后的效果
	//在设置上面的效果
	FrameSwitch.getInstance().contentNavClick($contentNavA, $contenFrame, "navclick");
	FrameSwitch.getInstance().navSettingHover($(".nav-setting"), $(".nav-setting i"), "change-normal", "change-bg");
	...
	// var arrow = new DropDown();
	DropDown.getInstance().publicMethod($contentTwoLeft, $leftArrow); //左箭头
	DropDown.getInstance().publicMethod($contentTwoLeft, $rightArrow); //右箭头
